...
class EsetController extends AppAdminController{
    ...
    public function initialize(){
        ...
        parent::initialize();
        // ページのタイトルセット
        $this->Erams->setTitle('テストセット編集ページ');
        // 下部ページで使うセッションを消去
        $session = $this->request->getSession();
        $session->delete('Erams.QuestionKey');
        // 必要なデータを確保
        if ( ! ($this->eset = $this->EramsData->getEsetByForm('/EsetTop')) ) {
            return;
        }
        // テストセットに含まれるセクションを確保
        $this->sections = $this->EramsDB->get('sections', 'subseq', [ 'eid' => $this->eset->id ] );
        // ビューに渡す情報をセット
        $this->set('Erams.Eset', $this->eset);
        $this->set('Erams.Sections', $this->sections);
        // 戻るページを設定
        $this->Erams->setBackPage('/EsetTop');
    }
    ...
    public function index() {
        ...
        // ログ出力
        $this->log( $tname.' Eset - index  eid : '.$this->eset->id.' mode: '.$this->eset->mode, 'info');
    }
    ...
    public function save(){
        ...
        $tname = $session->read('Erams.uname');
        // ログ出力
        $this->log( $tname.' Eset - save  eid : '.$this->eset->id.' mode: '.$this->eset->mode, 'info');
        
        // 編集モードか確認する
        if ( $this->eset->mode == 1 ) {
            $this->Flash->set("テスト提供中のテストセットに対する操作はできません。");
            $this->redirect('/Eset?eid='.$this->eset->id);
            return;
        }
    	// post 変数を取得
    ...
    public function createSec() {
        ...
        $tname = $session->read('Erams.uname');
        // ログ出力
        $this->log( $tname.' Eset - createSec  eid : '.$this->eset->id.' mode: '.$this->eset->mode, 'info');
    ...
    public function deleteSec() {
        ...
        if ( $this->eset->mode == 1 ) {
            // ログ出力
            $this->log( $tname.' Eset - deleteSec  eid : '.$this->eset->id.' mode: '.$this->eset->mode, 'info');
        ...
        // ログ出力
        $this->log( $tname.' Eset - deleteSec  eid : '.$this->eset->id.' mode: '.$this->eset->mode.' sid: '.$sid, 'info');
    ...
    public function swap() {
        ...
        if( $moveto != "u" && $moveto != 'd' ) {
            // ログ出力
            $this->log( $tname.' Eset - swap  eid : '.$this->eset->id.' mode: '.$this->eset->mode.' sid: '.$sid, 'info');
            
            $this->Flash->set("予期せぬエラーが発生しました。");
            $this->redirect('/Eset?'.'eid='.$eid);
            return;
        }
        // ログ出力
        $this->log( $tname.' Eset - swap  eid : '.$this->eset->id.
        ' mode: '.$this->eset->mode.' sid: '.$sid.' moveto: '.$moveto, 'info');
        // eid を用意
        $eid = $this->eset->id;
        // セクションを取得
